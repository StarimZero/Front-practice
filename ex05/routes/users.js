...
//마이페이지로 이동
router.get('/mypage', function (req, res) { //주소창에 직접치면 get이니까 이리로온다. 위에는 ejs가아니다...
	const uid = req.query.uid;
	const sql = "select * from users where uid=?";
	db.get().query(sql, [uid], function (err, rows) {
		res.render('index.ejs', {
			title: '내정보', pageName: 'users/mypage.ejs',
			user : rows[0]
		}); //PageName에 	
	});
});
...
router.post('/update', function(req,res){
	...
	const sql = "update users set uname=?, phone=?, add1=?, add2=? where uid=?";
	db.get().query(sql, [uname, phone, add1, add2, uid], function(err, rows){
		res.redirect('/');
	});
});
